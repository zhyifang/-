...
#define DEVICE_NAME                     "Nordic_UARTa"                               /**< Name of device. Will be included in the advertising data. */
...
void ble_init(void)
{
    uint32_t err_code;
    // Initialize.
    APP_TIMER_INIT(APP_TIMER_PRESCALER, APP_TIMER_MAX_TIMERS, APP_TIMER_OP_QUEUE_SIZE, false);   //Source Device时钟初始化
    ble_stack_init();                                                                            //蓝牙协议栈初始化
    gap_params_init();                                                                           //GAP initialization
    services_init();                                                                             //服务初始化
    advertising_init();                                                                          //广播参数初始化
    conn_params_init();                                                                          //initializing the Connection Parameters module.
