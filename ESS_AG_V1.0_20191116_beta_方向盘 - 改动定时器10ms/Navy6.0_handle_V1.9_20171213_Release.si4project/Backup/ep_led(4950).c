...
void LED_Display(void)
{
		switch(LCD_Handle.Menu)
		{
			case LCD_Menu_Main:
				...
				else //connected ok
				{
					...
					if(SYS_Handle.sys_state == SYS_State_Normal)
					{
						...
						{
							LED_DisplayPercent(THROTTLE_Handle.percent/31,THROTTLE_Handle.dir);
						}
						BuzzerHandle.g_Buzzer_on_flag = false;
					}
			...
			break;	
			case LCD_Menu_ThCal:
				if(THROTTLE_Handle.state == THROTTLE_State_Cal_Start)//向左方向校准
				{
					LED_blink(1);
					#ifdef CALL_CHECK
				...
				else if(THROTTLE_Handle.state == THROTTLE_State_Cal_For)//零点
				{
					LED_blink(5);
					#ifdef CALL_CHECK
				...
				else if(THROTTLE_Handle.state == THROTTLE_State_Cal_Mid)//向右校准
				{
					LED_blink(9);
					#ifdef CALL_CHECK
			...
			case LCD_Menu_MiddlePosition:
				if(Comm_ReadState() == Comm_State_Disconnected)//未连接，慢闪
				{
					...
					LED_DisplayPercent(THROTTLE_Handle.percent/31,THROTTLE_Handle.dir);//显示当前的角度挡位
			break;
