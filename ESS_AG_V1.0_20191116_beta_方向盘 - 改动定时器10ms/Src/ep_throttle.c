...
{	
	...
	if(handle->Cal_BackwardAngle < handle->Cal_ForwardAngle) //逆时针减角度，顺时针增加度。--------左边forward<360--359(0)-backward>0-----
	{
		if(ABS(handle->angle - handle->preAngle)>MAX_ANGLE_TURN)
		{
			//MAX_ANGLE=360,MAX_ANGLE-MAX_ANGLE_TURN==360-30
		...
		if(handle->Cal_ForwardAngle <= 225)//zero->f
		{
			...
			{
				handle->dir = 1;
				handle->PercentTemp = (127L*(handle->zeroAngle - handle->angle))/HALF_RANGE;//HALF_RANGE==135
			}
			else if((handle->angle > handle->zeroAngle) && (handle->angle <= MAX_ANGLE))
			{
				handle->dir = 0;
				handle->PercentTemp = (127L*(handle->angle - handle->zeroAngle))/HALF_RANGE;//HALF_RANGE==135
			}
			else
			{
				handle->dir = 0;
				handle->PercentTemp = (127L*(handle->angle + MAX_ANGLE - handle->zeroAngle))/HALF_RANGE;//HALF_RANGE==135
			}
		}
		else      //zero->b
		{	
			...
			{
				handle->dir = 1;
				handle->PercentTemp = (127L*(MAX_ANGLE - handle->angle + handle->zeroAngle))/HALF_RANGE;  //HALF_RANGE==135
			}
		}
	}
...
{	
	...
	{
		handle->state = THROTTLE_State_Cal_Mid;     //THROTTLE_State_Cal_For;
		#ifdef CALL_CHECK
	...
	else if(handle->state == THROTTLE_State_Cal_Mid)//THROTTLE_State_Cal_For)
	{
		handle->state = THROTTLE_State_Cal_For;     //THROTTLE_State_Cal_Mid;
		#ifdef CALL_CHECK
	...
	else if(handle->state == THROTTLE_State_Cal_For)//THROTTLE_State_Cal_Mid)
	{
